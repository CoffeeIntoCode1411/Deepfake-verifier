...
[data-testid="stFileUploaderDropzoneInstructions"] span:first-child::after {
    content: "Attach an image or video to verify \2014  or drag & drop it here";
...
def calculate_authenticity_score(fake_prob, pred_label, metadata_info, weather_info, fraud_ring_flag,
    ...
    # Verdict thresholds: >80 → Real/Authentic, 60–80 → Manual Review,
    # <60 → Fake/High Fraud Risk.
    if final_score > 80:
...
def _pdf_score_chart(score: float):
    ...
    ax.axvline(60, color="#9CA3AF", linestyle="--", linewidth=1)
    ax.axvline(80, color="#9CA3AF", linestyle="--", linewidth=1)
    ax.text(60, 0.42, "60", ha="center", fontsize=8, color="#6B7280")
    ...
    ax.set_yticks([]); ax.set_xlabel("Authenticity Score  (0–60 Fake · 60–80 Manual Review · 80–100 Real)", fontsize=8.5)
...
def generate_pdf_report(r: dict) -> bytes:
    ...
    if status.startswith("PASSED"):
        ...
        conclusion = (
            ...
            f"60–80 point range. While the evidence gathered was not conclusive enough to classify this content "
        ...
        conclusion = (
            f"This {media_word} received an authenticity score of {r['score']}/100, falling below the 60-point "
        ...
        <div class="verdict-hero {theme['cls']}">
            <div class="vh-top">
                ...
                <div class="vh-title-block">
                    ...
                    <div class="vh-sub">{r['pred_label']} · {theme['risk']}</div>
                </div>
                <div class="vh-score">
...
if not st.session_state.is_analyzed:
    ...
    if st.session_state.is_scanning:
        ...
        def process_single_file(uploaded_file, progress_cb=None):
            ...
            # --- Step 3: scene plausibility, context, and duplicate checks ---
            _tick(2)
            if not is_image:
        ...
        for r in results:
            ...
            with st.expander("🔍 View Full Forensic Breakdown", expanded=False):
                ...
                metric_defs = [
                    ...
                    (m2, "Deepfake Confidence", f"{round(r['fake_prob']*100,1)}%"),
                    (m3, "Real Confidence", f"{round(r['real_prob']*100,1)}%"),
                ...
                TAB_LABELS = ["🚨 Explainability", "🔬 File Forensics", "💥 Scene Plausibility",
                              "🌐 Context & Duplicate Check", "📤 Export Report"]
